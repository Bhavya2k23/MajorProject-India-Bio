...
export default function AnimalDetail() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${API_BASE}/animals/${id}`).then((r) => r.json()).catch(() => ({ success: false })),
      fetch(`${API_BASE}/animals/recommendations/${id}`).then((r) => r.json()).catch(() => ({ success: false })),
    ])
      ...
      .catch(() => { if (isMounted) setError("Failed to load species details"); })
  ...
  if (error || !species) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center pt-20 gap-4">
        ...
        <h2 className="text-2xl font-bold text-destructive">{error || "Species not found"}</h2>
      ...
      <div className="container mx-auto px-4 max-w-4xl pt-24">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          >
            ...
            {isFav ? "Saved to Favorites" : "Save Species"}
          </Button>
        </div>
        ...
        <div className="animate-slide-up bg-card border border-border p-8 md:p-12 rounded-3xl shadow-sm mb-8 text-center space-y-6">
          <div className="flex flex-wrap items-center justify-center gap-2">
            ...
            <span className="px-4 py-1.5 rounded-full text-xs font-medium bg-secondary text-secondary-foreground border border-border shadow-sm">
              Animal
            </span>
          </div>
          ...
          <p className="text-lg text-muted-foreground leading-relaxed max-w-2xl mx-auto">
            {species.description || "Detailed biological description is currently not available for this species."}
          </p>
        </div>
            ...
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Discover more wildlife sharing similar ecosystems or zones.
                </p>
              </div>
